...
Write-Host "Starting SOC Compliance Platform Services (Direct Mode)" -ForegroundColor Cyan
...
function Start-Service {
    ...
    if (-not (Test-Path $envFile)) {
        ...
        Write-Host "  Run configure-dev-environment.ps1 first" -ForegroundColor Yellow
        return $false
    }
    ...
    Push-Location $servicePath
    $startCmd = "npm run start:direct"
    $process = Start-Process powershell -ArgumentList "-NoExit", "-Command", $startCmd -PassThru -WindowStyle Minimized
    Pop-Location
...
foreach ($svc in $dockerServices) {
    ...
    if ($isRunning) {
        ...
        Write-Host "  MISSING: $svc is NOT running" -ForegroundColor Red
    }
}
