...
import { EmailProvider, EmailProviderService } from './email.provider';
...
describe('EmailProviderService', () => {
  ...
  beforeEach(() => {
    ...
    (AWS.SES as unknown as jest.Mock).mockImplementation(() => mockSES);
  ...
  describe('configure', () => {
    ...
    it('should throw error if SendGrid API key is missing', async () => {
      ...
      await expect(service.configure(config)).rejects.toThrow('SendGrid API key is required');
    });
  });
  ...
  describe('send', () => {
    describe('SMTP', () => {
      ...
      it('should handle send failures', async () => {
        mockSMTPTransporter.sendMail.mockRejectedValueOnce(new Error('SMTP connection failed'));
