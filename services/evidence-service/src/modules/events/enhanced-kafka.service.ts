...
export interface EnhancedEvidenceEvent {
  id?: string;
  eventType: string;
  entityId?: string;
  evidenceId?: string;
...
export class EnhancedKafkaService implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly documentProcessingStatus = new Map<string, DocumentProcessingStatus>();
  private readonly MESSAGE_DEDUP_TTL = 3600000; // 1 hour
  
  private readonly groupId = 'evidence-service-consumer';
  ...
  private readonly topics = {
    ...
    // Dead letter queues
    DLQ_EVIDENCE: 'dlq-evidence',
    DLQ_DOCUMENTS: 'dlq-documents',
  ...
  constructor(private configService: ConfigService) {
    ...
    this.producer = this.kafka.producer({
      ...
      transactionalId: `evidence-service-${process.env.POD_NAME || 'local'}`,
    });
    ...
    this.consumer = this.kafka.consumer({ 
      ...
      maxBytesPerPartition: 10485760, // 10MB for large documents
      retry: {
        retries: 5,
      },
    });
  ...
  private initializeCircuitBreaker() {
    const options = {
      ...
      name: 'evidence-kafka-producer',
      fallback: async (event: EnhancedEvidenceEvent) => {
    ...
    this.circuitBreaker = new CircuitBreaker(
      async (event: EnhancedEvidenceEvent) => this.publishEventInternal(event),
      options
    );

    this.circuitBreaker.on('open', () => {
      this.logger.error('Circuit breaker is now open');
      this.publishSystemAlert('CIRCUIT_BREAKER_OPEN', { service: 'evidence-service' });
    });
  ...
  async onModuleInit() {
    ...
    try {
      ...
      // Start document processing status cleanup
      this.startDocumentStatusCleanup();

      this.logger.log('Enhanced Evidence Kafka service initialized successfully');
    } catch (error) {
      this.logger.error('Failed to initialize Evidence Kafka service', error);
  ...
  private async createTopics() {
    const topicConfigs = [
      {
        ...
        configEntries: [
          ...
          { name: 'max.message.bytes', value: '10485760' }, // 10MB for documents
        ],
      },
      {
        topic: this.topics.DOCUMENT_PROCESSING,
        numPartitions: 5,
        replicationFactor: 1,
        configEntries: [
      ...
      {
        topic: this.topics.DOCUMENT_INDEXING,
        numPartitions: 2,
        replicationFactor: 1,
        configEntries: [
      ...
      {
        topic: this.topics.DLQ_EVIDENCE,
        numPartitions: 1,
        replicationFactor: 1,
        configEntries: [
      ...
      {
        topic: this.topics.DLQ_DOCUMENTS,
        numPartitions: 1,
        replicationFactor: 1,
        configEntries: [
    ...
    try {
      ...
      this.logger.log('Evidence Kafka topics created/verified');
    } catch (error) {
  ...
  private async subscribeToTopics() {
    const topics = [
      ...
      this.topics.DOCUMENT_VERIFICATION,
    ...
    try {
      ...
      switch (topic) {
        ...
        case this.topics.WORKFLOW_EVENTS:
          ...
          break;
        case this.topics.DOCUMENT_VERIFICATION:
  ...
  private async handleAuthEvent(event: any) {
    ...
    if (event.eventType === 'user.permissions.updated') {
      this.logger.debug('Updating evidence access permissions for user:', event.userId);
  ...
  private async handleClientEvent(event: any) {
    ...
    if (event.eventType === 'organization.deleted') {
      this.logger.warn('Organization deleted, archiving evidence:', event.organizationId);
      // Archive all evidence for deleted organization
    }
  }

  private async handleControlEvent(event: any) {
    // Handle control evidence requirements
    if (event.eventType === 'control.evidence.required') {
      this.logger.debug('New evidence requirement for control:', event.controlId);
      // Create evidence collection task
    }
  }

  private async handlePolicyEvent(event: any) {
    // Handle policy compliance checks
    if (event.eventType === 'policy.compliance.check') {
      this.logger.debug('Policy compliance check triggered:', event.policyId);
      // Verify evidence meets policy requirements
    }
  }

  private async handleWorkflowEvent(event: any) {
    // Handle workflow evidence requests
    if (event.eventType === 'workflow.evidence.request') {
      this.logger.debug('Workflow requesting evidence:', event.workflowId);
      // Provide evidence to workflow
    }
  }

  private async handleDocumentVerification(event: any) {
    // Handle document verification results
    this.logger.debug('Document verification result:', event.documentId);
    const status = this.documentProcessingStatus.get(event.documentId);
    if (status) {
      status.status = event.verified ? 'completed' : 'failed';
      status.error = event.error;
    }
  }

  private async handleSagaEvent(event: any) {
    // Implement saga pattern handling for distributed transactions
    this.logger.debug('Handling saga event:', event);
    if (event.compensate) {
      // Rollback evidence changes
      await this.compensateEvidenceChanges(event);
    }
  }

  private async compensateEvidenceChanges(event: any) {
    // Implement compensation logic
    this.logger.warn('Compensating evidence changes for saga:', event.sagaId);
  }

  /**
   * Publish an evidence event with circuit breaker protection
   */
  async publishEvent(event: EnhancedEvidenceEvent): Promise<void> {
    ...
    // Add event metadata
    const enrichedEvent: EnhancedEvidenceEvent = {
      ...event,
      id: event.id || uuidv4(),
  ...
  private async publishEventInternal(event: EnhancedEvidenceEvent): Promise<RecordMetadata[]> {
    ...
    if (transaction) {
      ...
      try {
        const result = await txn.send({
          topic: this.topics.EVIDENCE_EVENTS,
          messages: [{
            key: event.evidenceId || event.entityId || event.id,
   ...
   */
  async publishDocumentProcessing(
    documentId: string,
    operation: 'index' | 'verify' | 'extract' | 'convert',
    metadata: any,
  ): Promise<void> {
    const event = {
      id: uuidv4(),
    ...
    this.documentProcessingStatus.set(documentId, {
      documentId,
      status: 'pending',
      progress: 0,
    });
    ...
    this.logger.debug(`Document processing request published: ${documentId} - ${operation}`);
  }
  ...
  getDocumentStatus(documentId: string): DocumentProcessingStatus | undefined {
    return this.documentProcessingStatus.get(documentId);
  }

  /**
   * Batch publish events for better performance
   */
  async publishEventsBatch(events: EnhancedEvidenceEvent[]): Promise<void> {
    if (this.configService.get('DISABLE_KAFKA') === 'true') {
      return;
    }
    ...
    const topicMessages: TopicMessages[] = [
      {
        ...
        messages: events.map(event => ({
          key: event.evidenceId || event.entityId || event.id || uuidv4(),
    ...
    try {
      ...
      this.logger.debug(`Published batch of ${events.length} evidence events`);
    } catch (error) {
  ...
  private createMessageHeaders(event: EnhancedEvidenceEvent): Record<string, string> {
    ...
    if (event.documentMetadata) {
      headers['document-filename'] = event.documentMetadata.fileName || '';
      headers['document-mimetype'] = event.documentMetadata.mimeType || '';
      headers['document-checksum'] = event.documentMetadata.checksum || '';
    }
  ...
  private async sendToDeadLetterQueue(payload: EachMessagePayload): Promise<void> {
    try {
      const dlqTopic = payload.topic.includes('document') 
        ? this.topics.DLQ_DOCUMENTS 
        : this.topics.DLQ_EVIDENCE;
  ...
  private async queueEventForRetry(event: EnhancedEvidenceEvent): Promise<void> {
  ...
  private startDocumentStatusCleanup(): void {
    // Clean up old document processing statuses
    setInterval(() => {
      const cutoffTime = Date.now() - 86400000; // 24 hours
      for (const [id, status] of this.documentProcessingStatus.entries()) {
        if (status.status === 'completed' || status.status === 'failed') {
          this.documentProcessingStatus.delete(id);
        }
      }
