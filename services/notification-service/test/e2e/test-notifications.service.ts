...
export class TestNotificationsService {
  ...
  async sendNotification(createDto: any) {
    ...
    if (template.variables && template.variables.length > 0) {
      const missingVars = template.variables.filter(v => !createDto.data || !(v in createDto.data));
    ...
    if (createDto.recipients.includes('11111111-1111-1111-1111-111111111111') && 
  ...
  async getNotifications(query: any) {
    ...
    if (query.recipientId) {
      filteredNotifications = filteredNotifications.filter(n => n.recipientId === query.recipientId);
    }
    if (query.status) {
